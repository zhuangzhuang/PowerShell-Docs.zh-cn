...
包含操作系统中当前使用的区域性的名称。 区域性确定项的显示格式（例如数字、货币和日期），并将其存储在 **system.exception 对象中。** 用于 `Get-Culture` 显示计算机的区域性。 `$PSCulture` 包含 **名称** 属性的值。
...
| 属性                  | 说明                                   |
| ------------------------- | --------------------------------------------- |
| **BuildVersion**          | 当前版本的生成号       |
| **CLRVersion**            | 公共语言运行时的版本    |
|                           |  (CLR)                                          |
| **PSCompatibleVersions**  | 兼容的 PowerShell 版本    |
|                           | 与当前版本                      |
| **PSEdition**             | 此属性的值为 "Desktop"， |
|                           | Windows Server 和 Windows 客户端版本。   |
|                           | 此属性的值为 "Core"     |
|                           | 在 Nano Server 或下运行的 PowerShell       |
|                           | Windows IOT。                                  |
