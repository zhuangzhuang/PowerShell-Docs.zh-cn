...
在脚本块中，第一个命令使用 cmdlet 检索所有正在运行 `Where-Object` 的服务。 服务通过管道传输到 `ForEach-Object` cmdlet。 该 `ForEach-Object` cmdlet 将创建一个新的 [`[System.Management.Automation.CompletionResult]`](/dotnet/api/system.management.automation.completionresult) 对象，并使用当前服务的值填充该对象， (由管道变量 `$_`) 表示。
